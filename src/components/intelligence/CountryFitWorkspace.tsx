...
export default function CountryFitWorkspace({ countries, snapshot, initialContext = EMPTY, initialCountry = "portugal" }: { countries: CountryIdentity[]; snapshot: IntelligenceSnapshot; initialContext?: ResearchContext; initialCountry?: string }) {
  ...
  return <section className="ig-fit" aria-label="Country Fit and personal research plan">
    <div className="ig-panel"><p className="ig-eyebrow">01 / Your context</p><h2>A plan that starts with you.</h2><p className="ig-small">This is a research planner, not an automated country ranking. Changes here stay on this page; use your profile to save your goals.</p>
      ...
      </div><Link href="/onboarding" className="ig-link">Save goals through my TGPI profile →</Link>
    </div>
