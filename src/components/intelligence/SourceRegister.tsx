...
export default function SourceRegister() {
  ...
  return <section id="source-review" className="ig-section ig-panel" aria-label="Source register and review queue">
    ...
    <div className="ig-stack">{DOSSIER_COUNTRIES.map(country => { const dossier = dossierFor(country); return <details className="ig-details" key={country}><summary>{getCountry(country)?.name} — {dossier.sources.length} sources</summary>{dossier.sources.map(source => {
      ...
      return <article className="ig-watch-row" key={source.id}><a className="ig-link" href={source.url} target="_blank" rel="noreferrer">{source.title} ↗</a><p className="ig-meta">{source.publisher} · {source.kind.replaceAll("-", " ")} · {source.status === "unconfirmed" ? "Content unconfirmed" : source.status === "limited" ? "Limited-scope content review" : "Content reviewed"}</p><p className="ig-small">{source.note}</p><p className="ig-meta">{source.reviewedAt ? `Reviewed ${source.reviewedAt}` : "No completed content review"} · {claims.length} linked summaries · {source.automaticCheck ? "Automatic signals enabled" : "Manual verification only"}{nextReviews.length ? ` · Next content review by ${nextReviews.sort()[0]}` : ""}</p>{source.reviewIssue ? <p className="ig-caution">Open review issue — action above. A previously reviewed summary is not independently recertified by the automated check.</p> : null}</article>;
