      ...
      <div className="tgpi-container relative">
        <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-center">
          <div className="tgpi-reveal">
            <p className="text-xs font-extrabold uppercase tracking-[0.26em] text-[var(--tgpi-gold-light)]">
              TGPI Decision OS
            </p>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/onboarding"
                className="inline-flex min-h-12 items-center justify-center rounded-2xl bg-[var(--tgpi-gold)] px-6 py-4 text-sm font-extrabold text-[var(--tgpi-navy)] shadow-[0_18px_45px_rgba(0,0,0,.28)] transition duration-300 hover:-translate-y-1 hover:bg-[#d1a644] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
              >
                Build your global profile
              </Link>
              <Link
                href="/dashboard"
                className="inline-flex min-h-12 items-center justify-center rounded-2xl border border-white/20 bg-white/5 px-6 py-4 text-sm font-extrabold text-white transition duration-300 hover:-translate-y-1 hover:border-[var(--tgpi-gold)] hover:text-[var(--tgpi-gold-light)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--tgpi-gold)]"
              >
                View dashboard
              </Link>
            </div>
              ...
              <div className="rounded-[24px] border border-white/10 bg-white/[0.055] p-5">
                ...
                <div className="mt-5 space-y-3">
                  {countries.map((country, index) => (
                    <Link
                      key={country.name}
                      href={`/countries/${country.name === "England" ? "united-kingdom" : country.name.toLowerCase()}`}
                      className="group grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.045] p-4 transition duration-300 hover:-translate-y-0.5 hover:border-[var(--tgpi-gold)]/55 hover:bg-white/[0.08]"
                    >
                      <span className={`h-2.5 w-2.5 rounded-full ${country.tone}`} />
                      <span>
                        <span className="block text-sm font-extrabold text-white">{index + 1}. {country.name}</span>
              ...
              <div className="grid gap-4">
                <div className="rounded-[24px] border border-white/10 bg-white/[0.055] p-5">
                  <div className="flex items-end justify-between gap-4">
                    <div>
                      <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-white/45">Global readiness</p>
                ...
                <Link
                  href="/passport"
                  className="group rounded-[24px] border border-white/10 bg-white/[0.055] p-5 transition duration-300 hover:-translate-y-1 hover:border-[var(--tgpi-gold)]/55 hover:bg-white/[0.08]"
                >
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-white/45">Next blocker</p>
                      <p className="mt-2 text-lg font-extrabold text-white">Document preparation</p>
                      <p className="mt-1 text-sm text-white/45">Passport · Translation · Deadline</p>
                    </div>
