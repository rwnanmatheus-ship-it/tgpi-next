...
export default function GlobalWorkspaceOS({ identity, model, onboardingCompleted = false, rank }: GlobalWorkspaceOSProps) {
  ...
  return (
    <main id="main-content" className="min-h-screen overflow-x-clip bg-[#040B13] text-white">
      <div className="mx-auto max-w-[1720px] lg:grid lg:grid-cols-[216px_minmax(0,1fr)] 2xl:grid-cols-[232px_minmax(0,1fr)]">
        <aside className="min-w-0 border-b border-white/10 bg-[#06101B] px-4 py-4 lg:min-h-screen lg:border-b-0 lg:border-r lg:py-7">
        ...
        <div className="min-w-0 p-4 sm:p-6 xl:p-7 2xl:p-8">
          ...
          <header className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex min-w-0 items-center gap-3"><form action="/search" className="relative min-w-0 flex-1 xl:w-[360px] 2xl:w-[410px]" role="search"><label className="sr-only" htmlFor="workspace-search">Search TGPI</label><span aria-hidden="true" className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8999AB]">⌕</span><input className="h-12 w-full rounded-2xl border border-white/10 bg-[#0A1521] pl-10 pr-4 text-sm text-white outline-none transition placeholder:text-[#6F8195] focus:border-[#E5B94B]/55 focus:ring-4 focus:ring-[#E5B94B]/10" id="workspace-search" name="q" placeholder="Search countries, courses and resources" /></form><Link aria-label="Open notifications" className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-white/10 bg-[#0A1521] text-lg transition hover:border-[#E5B94B]/45 hover:bg-[#E5B94B]/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E5B94B]" href="/notifications" title="Notifications"><span aria-hidden="true">🔔</span></Link><Link aria-label="Open account settings" className="shrink-0 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E5B94B]" href="/profile/security"><WorkspaceAvatar identity={identity} /></Link></div>
          </header>
          ...
          <div className="mt-5 grid min-w-0 gap-5 2xl:grid-cols-[minmax(0,1fr)_minmax(300px,340px)]">
            ...
            <aside className="grid min-w-0 content-start gap-5">
              ...
              <section aria-labelledby="identity-card-title" className="min-w-0 overflow-hidden rounded-[28px] border border-white/10 bg-[#0A1521] p-5 sm:p-6"><div className="flex min-w-0 items-center gap-4"><WorkspaceAvatar identity={identity} large /><div className="min-w-0 flex-1"><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#E5B94B]">Your global identity</p><h2 id="identity-card-title" className="mt-1 truncate text-lg font-extrabold">{identity.fullName}</h2><p className="mt-1 truncate text-xs text-[#91A2B5]">{identity.headline || "Complete your public headline"}</p></div></div><div className="mt-5 flex min-w-0 flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#8798AA]">Profile readiness</p><p className="mt-1 text-xl font-extrabold">{identity.profileCompletion}%</p></div><span className="text-right text-[11px] font-extrabold text-[#AEE0C5]">{identity.membership}</span></div><div className="mt-3"><ProgressBar label="Global profile completeness" value={identity.profileCompletion} /></div><p className="mt-4 break-all text-[11px] text-[#7C8EA2]">{identity.globalId}</p><div className="mt-4 grid gap-2 sm:grid-cols-2"><Link className="rounded-xl border border-white/10 px-3 py-3 text-center text-xs font-extrabold text-white hover:border-[#E5B94B]/35" href="/profile/security">Edit profile</Link><Link className="rounded-xl bg-[#E5B94B] px-3 py-3 text-center text-xs font-extrabold text-[#07182D]" href={identity.publicProfileUrl}>View profile</Link></div></section>
